...
import { createDiagnosticSubmission } from "../lib/api";
...
export default function Diagnostic() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Attribution: UTM params, referrer, user agent.
    const params = new URLSearchParams(window.location.search);

    try {
      // The backend computes the result server-side and stores the row —
      // the client only ever sends raw inputs, never a computed result.
      const data = await createDiagnosticSubmission({
      ...
      setResult({
        tier: data.result_tier,
        status: data.result_status,
        vulnerability: data.result_vulnerability,
        project: data.result_project,
        checklist: data.result_checklist,
      });
    } catch {
      ...
      <div className="max-w-3xl mx-auto border border-teal/15 bg-white/[0.02] p-10 max-sm:p-5 max-sm:border-0 max-sm:bg-transparent">
        ...
        {submitStatus === "error" && (
          <div className="mt-4 border border-danger/40 bg-danger/5 px-4 py-3 flex items-center gap-3">
            <span className="font-mono text-[13px] tracking-[0.15em] uppercase text-danger">Submission Failed</span>
            <span className="text-[13px] text-white/50">Could not reach the server — please try again.</span>
          </div>
        )}
